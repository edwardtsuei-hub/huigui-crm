...
- 历史薪资条和旧通知记录已完成只读导出和 dry-run；真实回填仍必须人工复核并单独授权。
- 生产导入和员工查看仍必须使用明确身份字段，不允许退回姓名授权。

## 后续历史回填 dry-run

本次 schema 收口后，已另行完成历史回填 dry-run，仍未写生产库：
...
2. 人工复核历史回填 dry-run，决定是否仅授权回填旧薪资条身份字段；若要回填 `publishBatchId`，必须先指定 `2026-05` 的发布批次。
