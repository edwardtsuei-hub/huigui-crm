...
- `blocked_waiting_for_history_identity_backfill_review`：旧薪资条和旧通知记录的 `publishBatchId / 身份字段` 已完成 dry-run；真实回填仍需人工复核和单独授权。
...
- 1 条历史薪资条为 `auto_update_candidate`，0 条人工身份冲突。
...
- `auto_update_candidate` 仍需确认对应使用者后才能写库。
- 旧薪资条和旧通知记录的 `publishBatchId` 回填不由身份 dry-run SQL 覆盖；必须先人工指定 `2026-05` 发布批次，再另行确认和授权。
