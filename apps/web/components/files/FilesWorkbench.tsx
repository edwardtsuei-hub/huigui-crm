...
function relatedEntityDisplay(file: FileSummary) {
  if (!file.relatedEntity?.id && !file.businessId) {
    return "";
  }

  const label = file.relatedEntity?.label || businessTypeLabel(file.relatedEntity?.type || file.businessType);
  const name = file.relatedEntity?.name || file.businessId || file.relatedEntity?.id;
...
export function FilesWorkbench() {
  ...
  const relatedTypeParam = searchParams.get("relatedType") ?? "";
  ...
  useEffect(() => {
    ...
    async function loadLibrary() {
      ...
      try {
        ...
        if (relatedTypeParam) {
          params.set("relatedType", relatedTypeParam);
        } else if (deferredRelatedType) {
          params.set("relatedType", deferredRelatedType);
        }
        if (relatedIdParam) params.set("relatedId", relatedIdParam);
        if (trashMode && deleteReasonFilter) params.set("deleteReason", deleteReasonFilter);
        if (updatedFrom) params.set("updatedFrom", updatedFrom);
        if (updatedTo) params.set("updatedTo", updatedTo);
        if (itemIdsParam) {
          params.set("itemIds", itemIdsParam);
        } else if (activeView === "favorites" && favorites.length) {
          ...
          <div className="files-toolbar__popover-wrap">
            ...
            {moreFiltersOpen ? (
              <div className="files-toolbar__popover">
                ...
                <label>
                  <span>关联对象</span>
                  <select value={relatedType} onChange={(event) => onRelatedTypeChange(event.target.value)}>
                    <option value="">全部业务</option>
                    {fileBusinessTypeOptions.map((option) => (
...
function buildTableItemMeta(item: SelectableLibraryItem) {
  ...
  const relatedLabel = relatedEntityDisplay(item);
  if (relatedLabel) {
    return `${fileTypeDisplayLabel(item)} · ${relatedLabel}`;
  }
...
export function FilesInspector({
  ...
  const relationLabelMap: Record<string, string> = {
    ...
    quotation: "关联报价",
    sales_order: "关联订单",
    inspection_order: "关联检测",
    product: "关联产品",
    project: "关联项目",
    contract: "关联合同"
  };
  const relationType = fileItem?.relatedEntity?.type?.toLowerCase() ?? "";
  const relationLabel = relationLabelMap[relationType] ?? (fileItem?.relatedEntity?.label ? `关联${fileItem.relatedEntity.label}` : "关联对象");
  const relationValue = fileItem ? relatedEntityDisplay(fileItem) || "暂无关联对象" : "暂无关联对象";
      ...
      <div className="files-inspector__body files-inspector__content">
        ...
        <section className="files-inspector__section files-inspector__section--actions">
          ...
          <div className="files-inspector__actions">
            ...
            {isFile && fileItem?.relatedEntity?.href && !trashMode ? (
              <button
                className="button secondary inline"
                onClick={() => window.open(fileItem.relatedEntity!.href!, "_self")}
                type="button"
              >
                前往业务
              </button>
      ...
      for (const file of files) {
        ...
        try {
          const uploaded = await uploadFileToCos(file, {
            businessType: relatedType || category || "files_center",
            businessId: relatedId || undefined,
              ...
              {moreSettingsOpen ? (
                <div className="files-disclosure__panel files-form-grid">
                  ...
                  <label>
                    <span>业务对象 ID</span>
                    <input
                      placeholder="粘贴客户、报价、订单、检测或产品 ID"
                      value={relatedId}
