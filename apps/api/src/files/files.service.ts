...
} satisfies Prisma.FileRecordSelect;
...
export class FilesService {
  ...
  async createUploadToken(dto: CreateUploadTokenDto, user: AuthenticatedUser) {
    const folderId = dto.folderId?.trim();
  ...
  async createFileRecord(dto: UploadCallbackDto, user: AuthenticatedUser) {
    const archiveTarget = await this.resolveBusinessArchiveTarget(dto, user);
    const requestedFolderId = dto.folderId?.trim() || null;
    let folderId = requestedFolderId;

    if (folderId) {
      await this.ensureFolderExists(folderId, user);
    } else if (archiveTarget) {
      folderId = await this.ensureBusinessArchiveFolder(archiveTarget, user);
    }

    const requestedVersionGroupId = dto.versionGroupId?.trim() || null;
    const versionSeed = await this.resolveVersionSeed({
      fileName: dto.fileName.trim(),
      versionGroupId: requestedVersionGroupId,
      archiveTarget,
      user
    });
    const versionGroupId = versionSeed.versionGroupId;
    let versionNumber = 1;

    if (versionGroupId) {
      versionNumber = versionSeed.nextVersionNumber;
    }

    const normalizedStatus =
      this.normalizeStatus(dto.status) ??
      (archiveTarget ? FileRecordStatus.ARCHIVED : FileRecordStatus.ACTIVE);
    const partition = await this.recordPartition.getWritableCreateData(user);
    const created = await this.prisma.fileRecord.create({
      data: {
        ...
        category: this.normalizeNullableText(dto.category ?? archiveTarget?.defaultCategory ?? dto.businessType),
        tagText: this.normalizeTags(dto.tagText),
        note: this.normalizeNullableText(
          dto.note ??
            (archiveTarget
              ? `自动归档到${archiveTarget.label}：${archiveTarget.folderLabel}`
              : undefined),
        ),
        businessType: this.normalizeNullableText(archiveTarget?.type ?? dto.businessType),
        businessId: this.normalizeNullableText(archiveTarget?.id ?? dto.businessId),
        relatedType: this.normalizeNullableText(archiveTarget?.type ?? dto.relatedType),
        relatedId: this.normalizeNullableText(archiveTarget?.id ?? dto.relatedId),
        folderId,
        status: normalizedStatus,
        isImportant: dto.isImportant ?? false,
        isArchived: archiveTarget ? true : normalizedStatus === FileRecordStatus.ARCHIVED,
        permissionScope: this.normalizeNullableText(dto.permissionScope),
        versionGroupId,
        versionNumber,
  ...
  private normalizeQuery(query: FileListQueryDto, user: AuthenticatedUser): NormalizedQuery {
    return {
      ...
      relatedType: this.normalizeBusinessType(query.relatedType) || query.relatedType?.trim() || "",
      relatedId: query.relatedId?.trim() || "",
      uploaderUserId:
  ...
  private getBusinessArchiveMeta(type: string) {
    const metaMap: Record<string, { label: string; rootFolder: string; defaultCategory: string; hrefPrefix: string }> = {
      CUSTOMER: {
        label: "客户",
        rootFolder: "客户档案",
        defaultCategory: "客户交付",
      ...
      CONTRACT: {
        label: "合同",
        rootFolder: "合同档案",
        defaultCategory: "合同文件",
        hrefPrefix: "/contracts",
      },
      PROJECT: {
        label: "项目",
        rootFolder: "项目档案",
        defaultCategory: "项目资料",
  ...
  private async resolveBusinessArchiveTarget(dto: UploadCallbackDto, user: AuthenticatedUser): Promise<BusinessArchiveTarget | null> {
    const type = this.normalizeBusinessType(dto.businessType ?? dto.relatedType);
    const id = (dto.businessId ?? dto.relatedId)?.trim();

    if (!type || !id) {
      return null;
    }

    const meta = this.getBusinessArchiveMeta(type);
    const label = await this.resolveBusinessDisplayName(type, id, user);
    return {
      type,
      id,
      label: meta.label,
      folderLabel: label,
      href: meta.hrefPrefix ? `${meta.hrefPrefix}/${encodeURIComponent(id)}` : "",
      defaultCategory: meta.defaultCategory,
    };
  }

  private async resolveBusinessDisplayName(type: string, id: string, user: AuthenticatedUser) {
    const fallback = `${this.getBusinessArchiveMeta(type).label} ${id}`;
    const partitionWhere = this.recordPartition.buildWhere(user);

    try {
      switch (type) {
        case "CUSTOMER": {
          const where = this.recordPartition.mergeWhere<Prisma.CustomerWhereInput>(
            { id },
            partitionWhere as Partial<Prisma.CustomerWhereInput>,
          );
          const item = await this.prisma.customer.findFirst({
            where,
            select: { customerName: true, companyName: true },
          });
          return item?.companyName || item?.customerName || fallback;
        }
        case "QUOTATION": {
          const where = this.recordPartition.mergeWhere<Prisma.QuotationWhereInput>(
            { id },
            partitionWhere as Partial<Prisma.QuotationWhereInput>,
          );
          const item = await this.prisma.quotation.findFirst({
            where,
            select: { quotationNo: true },
          });
          return item?.quotationNo || fallback;
        }
        case "SALES_ORDER": {
          const where = this.recordPartition.mergeWhere<Prisma.SalesOrderWhereInput>(
            { id },
            partitionWhere as Partial<Prisma.SalesOrderWhereInput>,
          );
          const item = await this.prisma.salesOrder.findFirst({
            where,
            select: { orderNo: true },
          });
          return item?.orderNo || fallback;
        }
        case "INSPECTION_ORDER": {
          const where = this.recordPartition.mergeWhere<Prisma.InspectionOrderWhereInput>(
            { id },
            partitionWhere as Partial<Prisma.InspectionOrderWhereInput>,
          );
          const item = await this.prisma.inspectionOrder.findFirst({
            where,
            select: { inspectionNo: true, title: true },
          });
          return item?.inspectionNo || item?.title || fallback;
        }
        case "PRODUCT": {
        ...
        case "CONTRACT": {
          const item = await this.prisma.contract.findFirst({
            where: {
              AND: [{ id }, partitionWhere as Prisma.ContractWhereInput],
            },
            select: { contractNo: true, contractName: true },
          });
          return item?.contractNo || item?.contractName || fallback;
        }
        default:
          return fallback;
      }
      ...
      this.logger.warn(
        `解析档案业务对象失败: ${type}/${id} - ${error instanceof Error ? error.message : "未知错误"}`,
      );
      return fallback;
    }
  }

  private async ensureBusinessArchiveFolder(target: BusinessArchiveTarget, user: AuthenticatedUser) {
    const meta = this.getBusinessArchiveMeta(target.type);
    const rootId = await this.ensureArchiveFolder({
      name: meta.rootFolder,
      parentId: null,
    ...
    return this.ensureArchiveFolder({
      name: target.folderLabel,
      parentId: rootId,
      category: target.defaultCategory,
      tagText: [target.label, target.id].join(", "),
      user,
    });
  }

  private async ensureArchiveFolder(params: {
    name: string;
    parentId: string | null;
    category: string;
    tagText: string;
    ...
    const existing = await this.prisma.fileFolder.findFirst({
      where: this.withPartition(params.user, {
    ...
    const partition = await this.recordPartition.getWritableCreateData(params.user);
    const created = await this.prisma.fileFolder.create({
      data: {
        name: params.name,
        parentId: params.parentId,
        category: params.category,
        tagText: this.normalizeTags(params.tagText),
        note: "系统自动归档资料夹",
    ...
    await this.touchFolderChain(params.parentId);
    return created.id;
  }

  private async resolveVersionSeed(params: {
    fileName: string;
    versionGroupId: string | null;
    archiveTarget: BusinessArchiveTarget | null;
    user: AuthenticatedUser;
  }) {
    const latestVersion: { id?: string; versionGroupId?: string | null; versionNumber: number } | null = params.versionGroupId
      ? await this.prisma.fileRecord.findFirst({
          where: this.withPartition(params.user, { versionGroupId: params.versionGroupId }),
          orderBy: [{ versionNumber: "desc" }, { createdAt: "desc" }],
          select: { versionNumber: true },
        })
      : params.archiveTarget
        ? await this.prisma.fileRecord.findFirst({
            where: this.withPartition(params.user, {
              businessType: params.archiveTarget.type,
              businessId: params.archiveTarget.id,
              fileName: params.fileName,
            }),
            orderBy: [{ versionNumber: "desc" }, { createdAt: "desc" }],
            select: { versionGroupId: true, id: true, versionNumber: true },
          })
        : null;
    ...
    return {
      versionGroupId:
        params.versionGroupId ??
        latestVersion.versionGroupId ??
        latestVersion.id ??
        null,
  ...
  private buildFileWhere(
    ...
    const isBusinessLookup = Boolean(options.relatedType || options.relatedId);
    ...
    } else if (options.view === "all" && !isBusinessLookup) {
      where.folderId = null;
    }
          ...
          ? [
              {
                OR: [
                  { relatedType: options.relatedType },
                  { businessType: options.relatedType }
                ]
              }
            ]
          : []),
  ...
  private serializeFile(file: {
    ...
    const businessType = this.normalizeBusinessType(file.businessType ?? file.relatedType);
    const businessId = file.businessId ?? file.relatedId;
    const relatedType = this.normalizeBusinessType(file.relatedType ?? file.businessType);
    const relatedId = file.relatedId ?? file.businessId;
    const businessMeta = relatedType ? this.getBusinessArchiveMeta(relatedType) : null;
    const relatedName =
      relatedId && businessMeta
        ? `${businessMeta.label} ${relatedId}`
        : relatedId ?? businessId ?? file.relatedType ?? file.businessType;
    return {
      id: file.id,
      ...
      note: file.note,
      businessType: businessType || file.businessType,
      businessId,
      relatedEntity:
        relatedType || relatedId
          ? {
              type: relatedType || file.relatedType,
              id: relatedId,
              label: businessMeta?.label ?? file.relatedType,
              name: relatedName,
              href:
                relatedId && businessMeta?.hrefPrefix
                  ? `${businessMeta.hrefPrefix}/${encodeURIComponent(relatedId)}`
                  : null
            }
          : null,
